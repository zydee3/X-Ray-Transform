...
@njit(fastmath=True)
def ddx(f, x: int, epsilon: float = power(2, -12)) -> float:
    return (8 * (f(x + epsilon) - f(x - epsilon)) +
            f(x - 2 * epsilon) - f(x + 2 * epsilon)) / (12 * epsilon)


@njit(fastmath=True)
def dddx(f, x: int, epsilon: float = power(2, -8)):
...
@njit(fastmath=True)
def ddx(f, x: int, y: int, epsilon: float = power(2, -12)):
    return (8 * (f(x + epsilon, y) - f(x - epsilon, y)) +
            f(x - 2 * epsilon, y) - f(x + 2 * epsilon, y)) / (12 * epsilon)


@njit(fastmath=True)
def dddx(f, x: int, y: int, epsilon: float = power(2, -8)):
...
@njit(fastmath=True)
def ddy(f, x: int, y: int, epsilon: float = power(2, -12)):
    return (8 * (f(x + epsilon, y) - f(x - epsilon, y)) +
            f(x - 2 * epsilon, y) - f(x + 2 * epsilon, y)) / (12 * epsilon)


@njit(fastmath=True)
def dddy(f: Callable, x: int, y: int, epsilon: float = power(2, -8)):
