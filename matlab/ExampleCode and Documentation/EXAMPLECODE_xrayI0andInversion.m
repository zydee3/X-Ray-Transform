...
%% initialization
    ...
    % begin by initializing a surface (see EXAMPLECODE_initializeSurface)
        ...
        rsurf = RiemannSurface(dc,ms, stepSize = 0.02, stepType = 'RK4');


    % initialize some functions
        
        func0 = @(x,y) double( (((x-0).^2 + (y-0.5).^2) < 0.2) - (((x-0.3).^2 + (y-0.5).^2) < 0.2) ); 
...
%% I0
    ...
    % plot XrayI0
        ...
        pl = pcolor(betaG,alphaG,I0f(betaG,alphaG));
        pl.EdgeColor = 'none';
        title('I0')
...
%% I0 Inversion (I0perp*, geoA*, Hilbert, geoA of I0)
    ...
    % initialize points to transform over and precompute scattering relation
        beta = linspace(0,2*pi,250);
        alpha = linspace(-pi,pi,250)*0.5;   alpha = alpha(2:end-1);
    ...
    % geoHilbert
        disp('H');    
        hData = rsurf.geoHilbert(betaO,alphaO,AI0f, 400);
    ...
    % I0perpstar
        ...
        [VX,VY] = ndgrid(dc.aabbgrid(80));
        
        disp('Backproject I0_perp^*');   
        rsurf.stepType = 'IE';
        funcData = rsurf.I0perpstar(AstarHAI0f, VX,VY, 100)/(8*pi);   
